...
public class SystemDeleteFileTests
{
    ...
    [Test]
    [Skip("Requires System Delete Administrator account (elevated privileges not available)")]
    public async Task Can_System_Delete_A_File_Using_Signatory()
    {
        await using var fxFile = await TestFile.CreateAsync();
        await using var client = await TestNetwork.CreateClientAsync();
        var systemAddress = TestNetwork.SystemDeleteAdminAddress;
    ...
    [Test]
    [Skip("Requires System Delete Administrator account (elevated privileges not available)")]
    public async Task Can_System_Delete_A_File_And_Get_Record()
    {
        await using var fxFile = await TestFile.CreateAsync();
        await using var client = await TestNetwork.CreateClientAsync();
        var systemAddress = TestNetwork.SystemDeleteAdminAddress;
        ...
        await Assert.That(record.TransactionId.Payer).IsEqualTo(systemAddress);
    ...
    [Test]
    [Skip("Requires System Delete Administrator account (elevated privileges not available)")]
    public async Task Can_System_Delete_A_File_And_Get_Record_Using_Signatory()
    {
        await using var fxFile = await TestFile.CreateAsync();
        await using var client = await TestNetwork.CreateClientAsync();
        var systemAddress = TestNetwork.SystemDeleteAdminAddress;
    ...
    [Test]
    [Skip("Requires System Delete Administrator account (elevated privileges not available)")]
    public async Task Can_Not_Schedule_System_Delete()
    {
        await using var fxFile = await TestFile.CreateAsync();
        ...
        {
            await client.ScheduleAsync(new SystemDeleteFileParams
            {
                File = fxFile.CreateReceipt!.File
            });
        }).ThrowsException();
